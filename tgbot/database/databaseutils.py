    ...
    def save_user_links(
        ...
        try:
            with Session(self.engine) as session:
                stmt = (
                    select(TgLinkUsers)
                    .where(TgLinkUsers.id_user_tg == user_id, TgLinkUsers.url==link)
                )
        ...
        try:
            with Session(self.engine) as session:
                ...
                for link,data_link in zip(user_link,data_url):
                    ...
                    await get_and_update_data_from_notion(user_id=user_id,link=link,category=category,title=data_link["title"],priority=get_priority)
